      ...
      <main className="flex min-h-screen items-center justify-center px-4 py-12">
        <Card className="w-full max-w-md">
          <CardContent className="px-6 py-10 text-center">
            ...
            <h1 className="mt-4 text-2xl font-semibold tracking-tight">已收到 ¥{data.payable_money}</h1>
  ...
  return (
    <main className="min-h-screen px-4 py-8 sm:py-12">
      <div className="mx-auto max-w-4xl">
        <header className="mb-6 flex items-center justify-center gap-2 text-sm font-semibold">
          <WalletCards className="size-4 text-primary" />AliMPay 安全收银台
        </header>
            ...
            <CardContent className="px-5 py-6 sm:px-7">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <h1 className="mt-2 text-3xl font-semibold tracking-tight">¥{data.payable_money}</h1>
                </div>
                <Badge variant={checkoutExpired ? "danger" : "primary"}>
                  {checkoutExpired ? "收银台已过期，正在确认" : `剩余 ${countdown(Date.parse(data.expires_at) - now)}`}
                </Badge>
              </div>
              ...
              <div className="mt-6 flex min-h-[300px] items-center justify-center rounded-lg border p-5">
                {data.collection_mode === "business_qr" ? (
                  <img src={data.business_qr_url} alt="支付宝经营码" className="max-h-[270px] max-w-full object-contain" />
                ) : generatedQr ? (
                  <img src={generatedQr} alt="支付宝转账二维码" className="size-[270px] max-w-full object-contain" />
                ) : (
              ...
              <div className="mt-4 flex flex-col gap-2">
                ...
                <Button variant="outline" className="w-full text-xs" onClick={handleManualCheck} disabled={isChecking}>
                  <RefreshCw className={`size-3.5 mr-1 ${isChecking ? "animate-spin" : ""}`} />
              ...
              {checkoutExpired ? (
                <div className="mt-4 flex gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-3 text-xs leading-5 text-muted">
                  ...
                  5 分钟下单时间已过。如果你已经发起付款，请等待确认；系统会监控到 {formatDate(data.monitor_until)}。如果尚未付款，请返回商户重新下单。
                </div>
              ) : null}
            </CardContent>
          </Card>
              ...
              <CardContent className="px-5 py-5">
                ...
                <dl className="mt-4 space-y-4">
                  <div>
                    ...
                    <dd className="mt-1 break-all font-mono text-xs">{data.out_trade_no}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-muted">平台订单号</dt>
                    <dd className="mt-1 break-all font-mono text-xs">{data.trade_no}</dd>
                  </div>
                  <div>
                  ...
                  <div>
                    <dt className="text-xs text-muted">确认窗口</dt>
                    <dd className="mt-1 font-mono text-sm">{countdown(Date.parse(data.monitor_until) - now)}</dd>
                  </div>
                </dl>
              </CardContent>
            </Card>
