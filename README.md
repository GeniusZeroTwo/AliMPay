...
每个收银台有效 5 分钟，但订单会继续被有限监控至创建后第 7 分钟：

1. 没有待确认订单时，不请求支付宝。
2. 存在待确认订单时，全实例最多每 5 秒发起一轮合并账务查询，而不是每个订单单独查询。
3. 经营码按收入方向、唯一实付金额和发生时间匹配。
4. 转账模式额外要求 `trans_memo` 等于商户订单号。
5. 第 5–7 分钟到账记为 `late_paid`，对易支付接口仍映射为 `status=1` 并正常通知。
6. 第 7 分钟后停止自动匹配，过期订单与审计数据保留。
...
单元与契约测试覆盖 V1/V2 签名、订单幂等、金额分配、5/7 分钟状态机、账务流水去重、共享扫描、通知十次重试、会话与 CSRF、SSRF，以及桌面首次配置和移动端页面。
